...
const UserProfile = () => {
  ...
  useEffect(() => {
    // 백엔드로부터 유저 정보 가져오기
    axios
      .get("http://localhost:8088/api/userinfo")
      .then((response) => {
    ...
    // axios
    //   .get("http://localhost:8088/api/comments")
    //   .then((response) => {
  ...
  const handleSubmitUpdatedInfo = () => {
    axios
      .put("http://localhost:8088/api/userinfo", updatedInfo)
      .then((response) => {
        console.log("Updated info sent to server:", response.data);
        // 필요하다면 서버로부터 응답을 처리하는 로직 추가
      })
      .catch((error) => {
